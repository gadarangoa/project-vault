...
function Empty({ className, ...props }: ComponentProps<'div'>) {
  return <div data-slot="empty" className={cn('flex w-full min-w-0 flex-1 flex-col items-center justify-center gap-4 rounded-xl border border-dashed p-6 text-center', className)} {...props} />
}
...
const emptyMediaVariants = cva('mb-2 flex shrink-0 items-center justify-center [&_svg]:pointer-events-none [&_svg]:shrink-0', {
  variants: { variant: { default: 'bg-transparent', icon: 'flex size-10 items-center justify-center rounded-xl bg-muted text-muted-foreground [&_svg:not([class*=size-])]:size-5' } },
...
function EmptyContent({ className, ...props }: ComponentProps<'div'>) {
  return <div data-slot="empty-content" className={cn('flex w-full max-w-sm flex-col items-center gap-2.5 text-sm', className)} {...props} />
}
